...
import com.cj.weiboCrawler.utils.other.MidFormat;
...
public class Status extends DeepCrawler {
	...
	public void setTimeInternal(int timeInternal) {
		this.randomSecons = timeInternal;
	}
	...
	public Links visitAndGetNextLinks(Page page) {
		...
		try {
			Thread.sleep((Math.abs(random.nextInt()) % randomSecons) * 1000+fixSeconds*1000);
		...
		if (firstStatus.contains("没发过微博") && outDivs.size() == 3) {
			...
			for (int i = 0; i < outDivs.size() - 2; i++) {
				...
				statusCount++;
				curPageStatuses += "userId:" + userId + ",userName:" + userName + ",mid:" + mid + ",encrypted_mid:"
						...
						+ fetchTimeFormat + "\n";
				wbdb.insertStatus(this);
			}
		}
		allStatus += curPageStatuses;
		return null;
	}
	...
	public String getStatus(String uid, int statusNum) throws Exception {
		userId = uid;
		// allStatus = "";
		int actualPageNum = statusNum/10 +1;
		int end = actualPageNum < endPageNum ? actualPageNum : endPageNum;
