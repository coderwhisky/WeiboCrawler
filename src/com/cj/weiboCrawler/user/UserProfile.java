...
public class UserProfile extends DeepCrawler {
	...
	public Links visitAndGetNextLinks(Page page) {
		...
		try {
			Thread.sleep((Math.abs(random.nextInt()) % randomSeconds) * 1000+fixSeconds*1000);
		...
		if (!url.contains("info")) {
			...
			statusNum = Integer.parseInt(tmp1.substring(tmp1.indexOf("[") + 1, tmp1.indexOf("]")));
			...
			//头像地址
			//System.out.println(divcs.first().select("img").first().attr("src"));
			avatar=divcs.first().select("img").first().attr("src");//偶尔出现BUG！！！！！！
			
			//微博等级
			Element firstBlock = divcs.get(1);
			String Rank = firstBlock.select("a").first().text();
			uRank = Integer.parseInt(Rank.substring(0, Rank.indexOf("级")));//偶尔获取空字符串！！！！！！！
		...
		// uer最终结果
		result = "uid:" + uid + ",name:" + name + ",sex:" + sex +",uRank:" + uRank + ",statuseNum:" + statusNum
	...
	/*public static void main(String[] args) throws  Exception {
        ...
        myRequester3.setCookie("_T_WM=bbcc3ae1c86c90ede43a6112f11e83fa;SUB=_2A255syW3DeTxGeNN4lAS9yrMzD6IHXVbXEv_rDV6PUJbrdAKLW3ikW0X3mPhp9MnOdJY5V3pzvUP_TeLyg..;gsid_CTandWM=4uHs50b11w0WVQ3B1C7fsmCNM1a;M_WEIBOCN_PARAMS=rl%3D1;");
        myRequester3.setUserAgent("Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; MANMJS; rv:11.0) like Gecko");
        userProfile.startCrawler("1901064885");
	}*/
}